...
class Encoder(nn.Module):
    ...
    def __init__(self, in_channel=1, first_out_channel=4):
        ...
        self.conv0 = nn.Sequential(
            ConvBlock(in_channel, c), ConvInsBlock(c, 2 * c), ConvInsBlock(2 * c, 2 * c)
        )

        self.conv1 = nn.Sequential(
            nn.AvgPool3d(2), ConvInsBlock(2 * c, 4 * c), ConvInsBlock(4 * c, 4 * c)
        )

        self.conv2 = nn.Sequential(
            nn.AvgPool3d(2), ConvInsBlock(4 * c, 8 * c), ConvInsBlock(8 * c, 8 * c)
        )

        self.conv3 = nn.Sequential(
            nn.AvgPool3d(2), ConvInsBlock(8 * c, 16 * c), ConvInsBlock(16 * c, 16 * c)
        )

        self.conv4 = nn.Sequential(
            nn.AvgPool3d(2), ConvInsBlock(16 * c, 32 * c), ConvInsBlock(32 * c, 32 * c)
        )
...
class PositionalEncodingLayer(nn.Module):
    ...
    def forward(self, feat):
        ...
        emb_x = (
            torch.cat((sin_inp_x.cos(), sin_inp_x.sin()), dim=-1)
            .unsqueeze(1)
            .unsqueeze(1)
        )  # 160, 1, 1, 2
        emb_y = torch.cat((sin_inp_y.cos(), sin_inp_y.sin()), dim=-1).unsqueeze(
            1
        )  # 192, 1, 2
        emb_z = torch.cat((sin_inp_z.cos(), sin_inp_z.sin()), dim=-1)  # 224， 2
        emb = torch.zeros((x, y, z, self.channels * 3), device=feat.device).type(
            feat.type()
        )
...
class Im2grid(nn.Module):
    def __init__(
        self, inshape=(160, 192, 160), flow_multiplier=1.0, in_channel=1, channels=4
    ):
