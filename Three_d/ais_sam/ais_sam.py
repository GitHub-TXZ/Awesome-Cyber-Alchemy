...
class Gate(nn.Module):
    ...
    def forward(self, x):
        x = self.linear(x)  #(b,h,w,1)
        x = torch.sigmoid(x)
        x = rearrange(x, 'b h w c -> b (h w) c')
        return torch.mean(x, dim=1).unsqueeze(1).unsqueeze(2) #(b,n,1)
...
class AIS_SAM(nn.Module):
    def __init__(self, r: int=4, adapter_dim: int=64, spg=SPG(), d_size=20, **kwargs):
        ...
        checkpoint = "/home/tanxz/Codes/3DSAM-adapter/3DSAM-adapter/ais_sam/sam_med2d/checkpth/sam-med2d_b.pth"
        state_dict = torch.load(checkpoint, map_location='cpu')
        image_encoder.load_state_dict(state_dict['model'], strict=False)
        self.w_As = []
        self.w_Bs = []
