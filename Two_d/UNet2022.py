...
if __name__ == '__main__':
    ...
    model = unet2022(config,embedding_dim=embedding_dim, num_heads=[4, 8, 16, 32],deep_supervision=True ,num_input_channels=3,num_classes=2).to(device)
    # 生成伪3D数据测试模型
    input_shape = (1, 3, 224, 224)  # (batch_size, channels, height, width, depth)
    input= torch.randn(input_shape).to(device)
    print("input_shape: ", input.shape)
    output = model(input)
    print("output_shape: ", output[0].shape)
    print()
...
#
# # 使用view进行维度重塑
# viewed_tensor = x.view(2, 3, 2)
# print("\nTensor after view(2, 3, 2):\n", viewed_tensor)
#
# # 使用reshape进行维度重塑
# reshaped_tensor = x.reshape(3, 2, 2)
# print("\nTensor after reshape(3, 2, 2):\n", reshaped_tensor)
#
# # 使用permute改变维度顺序
# permuted_tensor = x.permute(1, 0, 2)
# print("\nTensor after permute(1, 0, 2):\n", permuted_tensor)
#
# # 使用unfold进行展开操作
# unfolded_tensor = x.unfold(dimension=1, size=2, step=1)
# print("\nTensor after unfold(dimension=1, size=2, step=1):\n", unfolded_tensor)
#
# # 使用chunk将张量分块
# chunks = x.chunk(chunks=2, dim=1)
# print("\nTensor after chunk(chunks=2, dim=1):")
# for i, chunk in enumerate(chunks):
#     print(f"Chunk {i}:\n", chunk)
#
# unbound_tensors = x.unbind(dim=1)
# print("\nTensor after unbind(dim=1):")
# for i, unbound in enumerate(unbound_tensors):
#     print(f"Unbound {i}:\n", unbound)
#
# stacked_tensor = torch.stack(unbound_tensors, dim=1)
# print("\nTensor after stacking unbound tensors:\n", stacked_tensor)
#
# # 展示原始张量的形状和变换后张量的形状
# print("\nShapes:")
...
# print("Chunked Shape:", chunks[0].shape)
# print("Stacked Shape:", stacked_tensor.shape)
#
#
...
# 使用unfold进行展开操作
# 选择适当的size和step参数以确保展开操作与fold兼容
unfolded_tensor = x.unfold(dimension=1, size=1, step=1).unfold(dimension=2, size=1, step=1)
unfolded_reshaped = unfolded_tensor.contiguous().view(1, -1, 1)
print("\nTensor after unfold and reshape:\n", unfolded_reshaped)

# 使用fold进行折叠操作
# fold的参数需要与unfold的参数相匹配
folded_tensor = fold(input=unfolded_reshaped, output_size=(2, 3), kernel_size=(1, 1), stride=(1, 1))
print("\nTensor after fold:\n", folded_tensor)

# 展示原始张量和变换后张量的形状
print("\nShapes:")
print("Original Shape:", x.shape)
print("Unfolded and Reshaped Shape:", unfolded_reshaped.shape)
