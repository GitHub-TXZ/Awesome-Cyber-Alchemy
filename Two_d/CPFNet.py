...
class CPFNet(nn.Module):
    def __init__(self, out_planes=1, ccm=True, norm_layer=nn.BatchNorm2d, is_training=True, expansion=2,image_size=512,
                 base_channel=32):
        ...
        if self.expansion == 4 and self.base_channel == 64:
            ...
            # expan = [int(image_size/4), int(image_size/2), image_size]
            # spatial_ch = [int(image_size/8), int(image_size/8)]
            # conv_channel_up = [int(image_size/4), int(image_size/2), image_size]
            expan = [128, 256, 512]
            spatial_ch = [64, 64]
    ...
    def forward(self, x):
        ...
        main_out = F.interpolate(main_out, size=[self.image_size,self.image_size], mode='bilinear', align_corners=True)
...
if __name__ == '__main__':
   # 生成一个随机的输入图像，形状为 (1, 3, 244, 244)
   image_size = 224
   device = torch.device("cuda:3" if torch.cuda.is_available() else "cpu")
   input_image = torch.rand(1, 3, image_size, image_size).to(device)
   model = CPFNet(image_size = image_size).to(device)
   # 使用模型进行推理
