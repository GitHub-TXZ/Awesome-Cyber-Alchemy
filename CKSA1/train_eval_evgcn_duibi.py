...
import torch
from opt import *
from EV_GCN_duibi import EV_GCN
...
if __name__ == '__main__':
    for asdw in [0.45,0.75,0.85,0.4,0.6,0.8,0.5,]:#0.1,0.15,0.20,0.25,0.3,0.35,0.55,0.65,0.7,0.9
            book_name_xls = f'双侧监督构图锚点 gl={asdw}.xls'
            ...
            regions1 = ["ALL"]
            #"M1", "M2", "M3", "M4", "M5","M1","M2", "M3", "M4", "M5", "M6", "L", "I", "C", "IC"
              ...
              for region in regions:
                  for k in xy:
                      if da == 1 or da == 0:
                        ...
                        dl = dataloader()
                        disease, sub, y,ids = dl.load_data(region) # imaging features (raw), labels, non-image data+
                        x = y
                        n_folds = 10
                        ...
                        for fold in range(n_folds):
                            ...
                            b = len(y[train_ind])
                            c = (b - a) / a
                            data = 1 - sum(y[test_ind]) / len(y[test_ind])
                            weights = torch.tensor([1, c], dtype=torch.float).to(opt.device)
                            edge_index, edgenet_input,edge_labels, train_labels = dl.get_PAE_inputs(region,train_ind)
                            ...
                            model = EV_GCN(node_ftr_dis.shape[1], opt.num_classes, dropout[region],
                                               ...
                                               lg=opt.lg, edgenet_input_dim=edgenet_input.shape[1] // 2, lg1=opt.lg1,gl=asdw)
                            ...
                            def train():
                                ...
                                for epoch in range(num_iter[region]):
                                    ...
                                    with torch.set_grad_enabled(True):
                                        node_logits,edge_weights = model(features_cuda_dis,features_cuda_hea, edge_index, edgenet_input)
                                        loss = loss_fn(node_logits[train_ind], labels[train_ind]) + 0.1 * dl.nn_loss(edge_weights[train_labels],one_shot[train_labels])
                                        loss.backward()
                                        optimizer.step()
                            ...
                            def evaluate():
                                ...
                                model.eval()
                                node_logits,_,_ = model(features_cuda_dis,features_cuda_hea, edge_index, edgenet_input)
                                logits_test = node_logits[test_ind].detach().cpu().numpy()
