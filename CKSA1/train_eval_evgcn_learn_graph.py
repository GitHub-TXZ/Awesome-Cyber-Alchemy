from opt import *
from EV_GCN_abs_del_learn_graph import EV_GCN
...
if __name__ == '__main__':
    for asdw in [0.75]:#0.1,0.15,0.20,0.25,0.3,0.35,0.55,0.65,0.7,0.9    0.45,0.75,0.85,0.4,0.6
            book_name_xls = f'消融实验_取消自学习构图 gl={asdw}.xls'
                      ...
                      if da == 1 or da == 0:
                        ...
                        for fold in range(n_folds):
                            ...
                            def train():
                                ...
                                for epoch in range(num_iter[region]):
                                    ...
                                    with torch.set_grad_enabled(True):
                                        node_logits, LR_logit = model(features_cuda_dis, features_cuda_hea, edge_index, edgenet_input, edge_mask)
                                        #topk_index = dl.find_k_largest_indices(20, train_ind, test_ind, val)
                            ...
                            def evaluate():
                                ...
                                model.eval()
                                node_logits,_, = model(features_cuda_dis,features_cuda_hea, edge_index, edgenet_input,edge_mask)
                                logits_test = node_logits[test_ind].detach().cpu().numpy()
