...
from utils.metrics import accuracy, auc, prf, save, auc_la,sa,kappa,to_icc,Over_all
...
if __name__ == '__main__':
    ...
    named = "映射维度"
    if a == 0:
        for proportion1 in [0.4]:#0.65# 0.1,0.15,0.20,0.25,0.3,0.35,0.55,0.65,0.7,0.9    0.45,0.75,0.85,0.4,0.6
            for asdw in [0.45]:
                print(proportion1)
                Over_all(named, proportion1)
    else:
        for proportion1 in [0.4]:#0.65#0.1,0.15,0.20,0.25,0.3,0.35,0.55,0.65,0.7,0.9    0.45,0.75,0.85,0.4,0.6
            for asdw in [0.45]:
                book_name_xls = f'讨论实验 proportion={proportion1}.xls'
                ...
                dropout = {"M1": 0.1, "M2": 0.1, "M3": 0.01, "M4": 0.01, "M5": 0.1, "M6": 0.1, "I": 0.1, "C": 0.1, "L": 0.1,
                           "IC": 0.1,"ALL":0.1}
                num_iter = {"M1": 1000, "M2": 1000, "M3": 1000, "M4": 1000, "M5": 1000, "M6": 1000, "I": 1000, "C": 1000, "L": 1000,
                            "IC": 1000,"ALL":1000}
                              ...
                              if da == 1 or da == 0:
                                ...
                                for fold in range(n_folds):
                                    ...
                                    model = EV_GCN(node_ftr_dis.shape[1], opt.num_classes, dropout[region],
                                                   ...
                                                   gl=asdw,all_n=all_n, weidu=weidu)
                                    ...
                                    fold_model_path = opt.ckpt_path + "/discuss_fold{}_region_{}_proportion_{}_epoch_{}_{}.pth".format(fold,region,proportion1, 300, weidu)
                                    ...
                                    def train():
                                        ...
                                        for epoch in range(num_iter[region]):
                                            ...
                                            with torch.set_grad_enabled(True):
                                                node_logits, edge_weights, LR_logit, val,index,weights = model(features_cuda_dis, features_cuda_hea,
                                                                                                               edge_index, edgenet_input, edge_mask)
                                                num_size = int(len(train_ind) * proportion1)
                                                topk_index = dl.find_k_largest_indices(num_size, train_ind, test_ind, val)
                                            ...
                                            with torch.set_grad_enabled(False):
                                                node_logits,edge_weights,LR_logit,_,_,_ = model(features_cuda_dis,features_cuda_hea, edge_index, edgenet_input,edge_mask)
                                    ...
                                    def evaluate():
                                        ...
                                        model.eval()
                                        node_logits,_,_,_,_,_ = model(features_cuda_dis,features_cuda_hea, edge_index, edgenet_input,edge_mask)
                                        logits_test = node_logits[test_ind].detach().cpu().numpy()
