...
import torch
from opt1 import *
from EV_GCN另一种方案LR监督 import EV_GCN
...
if __name__ == '__main__':
    for asdw in [0.5,0.4,]:#0.1,0.15,0.20,0.25,0.3,0.35,0.55,0.65,0.7,0.9    0.45,0.75,0.85,0.4,0.6 0.6, 0.8
            book_name_xls = f'双侧监督构图LR监督锚点 gl={asdw}.xls'
                  ...
                  for k in xy:
                      if da == 1 or da == 0:
                        ...
                        for fold in range(n_folds):
                            ...
                            features_cuda_sub = torch.tensor(node_ftr_all, dtype=torch.float32).to(opt.device)
                            ...
                            def train():
                                ...
                                for epoch in range(num_iter[region]):
                                    ...
                                    with torch.set_grad_enabled(True):
                                        node_logits,edge_weights,LR_logit = model(features_cuda_dis, features_cuda_hea, features_cuda_sub, edge_index, edgenet_input)
                                        LR_loss = loss_fn1(LR_logit[train_ind], labels_L[train_ind])
                                        loss = 0.8 * loss_fn(node_logits[train_ind], labels[train_ind]) + 0.1 * dl.nn_loss(edge_weights[train_labels],one_shot[train_labels]) + 0.1 * LR_loss
                                        loss.backward()
                                        optimizer.step()
