...
class EV_GCN(torch.nn.Module):
    def __init__(self, input_dim, num_classes, dropout, edgenet_input_dim, edge_dropout, hgc, lg,lg1,gl=0.5):
        ...
        for i in range(lg):
            in_channels = input_dim if i == 0 else hidden1[i - 1]
            self.gconv.append(tg.nn.ChebConv(int(in_channels), hidden[i], K, normalization='sym', bias=bias))
        cls_input_dim = sum(hidden) * 3
    ...
    def forward(self, features_dis,features_hea,features_sub, edge_index, edgenet_input, enforce_edropout=False):
        ...
        for i in range(1, self.lg1):
            ...
            h0_hea = jk
        features_sub = F.dropout(features_sub, self.dropout, self.training)
        h_sub = self.relu(self.gconv[0](features_sub, edge_index, edge_weight))
        h0_sub = h_sub
        for i in range(1, self.lg):
            h_sub = F.dropout(h_sub, self.dropout, self.training)
            h_sub = self.relu(self.gconv[i](h_sub, edge_index, edge_weight))
            jk = torch.cat((h0_sub, h_sub), dim=1)
            h0_sub = jk
        edge_logit, dis, hea = self.edge_pos(h0_dis, h0_hea)
        chayi = torch.abs(h0_dis - h0_hea)
        logit = self.cls(torch.cat((chayi, dis, h0_sub), dim=1))
        return logit,merged_tensor, edge_logit
